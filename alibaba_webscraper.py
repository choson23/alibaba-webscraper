...
albums = WebDriverWait(driver, 10).until(
    EC.presence_of_all_elements_located((By.XPATH, "/div[contains(@class, 'icbu-product-card vertical large product-item')]"))
)

albums = driver.find_elements(By.XPATH, "//img[contains(@class, 'product-image')]")

if albums:
    print(len(albums))

# Descargar la imagen usando la sesión
#mg_data = requests.get(url_imagen, headers=headers).content
